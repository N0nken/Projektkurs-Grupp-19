...
ifeq ($(OS),Windows_NT)                    # MSYS / Git‑Bash / mingw32‑make
    ...
    ifeq ($(UNAME_S),Darwin)               # macOS (Homebrew)
        PLATFORM   := MAC
        CC         := clang                
        CFLAGS_SDL := $(shell sdl2-config --cflags) $(shell pkg-config --cflags SDL2_image SDL2_net)
        LIBS_SDL   := $(shell sdl2-config --libs)   $(shell pkg-config --libs SDL2_image SDL2_net)
...
# ---------------- server.o -------------
$(BINDIR)/server.o: $(SRCDIR)/server.c $(BINDIR)/attacks.o $(BINDIR)/player.o $(BINDIR)/input_logger.o $(BINDIR)/movement.o
	$(MKDIR)
